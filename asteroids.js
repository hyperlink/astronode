// Original: http://github.com/erkie/erkie.github.com
// Modifed by Xiaoxin Lu (@hyperlink)
...
function Asteroids() {
	...
	*/
	...
	var isRunning = true;
	var lastUpdate = this.lastUpdate = new Date().getTime();
	
	this.update = function(override) {
		var forceChange = override == null ? false : override;
		
		// ==
		// logic
		// ==
		var nowTime = new Date().getTime();
		var tDelta = override == null ?  (nowTime - lastUpdate) / 1000 : FPS/1000;
		lastUpdate = nowTime;
		...
		if ( this.pos.x > w ) {
			...
			console.log('exited left side '+ this.pos.x);
			console.log('tDelta ', tDelta);
			console.log('before ', before);
			console.log('after ',after);
			window.scrollTo(this.scrollPos.x - 50, this.scrollPos.y);
	...
	this.shipExit = function(isRight) {
		if ( this.isConnected() ) {
			this.shipLeft = true;
			setTimeout(function() {
				console.log("ship has exited "+ (isRight ? "Right" : "Left"));
	...
	this.shipEnter = function(isRight, tel) {
		console.log("ENTER SHIP");
		this.shipLeft = false;
		var pos = tel.pos, dir = tel.dir, vel = tel.vel;
		console.dir(pos, dir);
		that.pos.x = isRight ? w : 0 ;
		that.pos.y = pos.y;
		that.dir.x = dir.x;
		that.dir.y = dir.y;
		that.vel.x = vel.x;
		that.vel.y = vel.y; 
		console.log(that.pos, that.dir);
		that.update.call(that, true);
		setTimeout(function(){
			interval = setInterval(updateFunc, 1000 / FPS);
		}, 1000 / FPS);
	}
